...
public class SocioJpaController implements Serializable {
    ...
    public void create(Socio socio) throws PreexistingEntityException, Exception {
        ...
        if (socio.getFineList() == null) {
            socio.setFineList(new ArrayList<Fine>());
        }
        ...
        try {
            ...
            for (Fine fineListFineToAttach : socio.getFineList()) {
            ...
            socio.setFineList(attachedFineList);
            ...
            for (Fine fineListFine : socio.getFineList()) {
                Socio oldSocioIdOfFineListFine = fineListFine.getSocioId();
                fineListFine.setSocioId(socio);
                fineListFine = em.merge(fineListFine);
                if (oldSocioIdOfFineListFine != null) {
                    oldSocioIdOfFineListFine.getFineList().remove(fineListFine);
                    oldSocioIdOfFineListFine = em.merge(oldSocioIdOfFineListFine);
                }
            }
    ...
    public void edit(Socio socio) throws IllegalOrphanException, NonexistentEntityException, Exception {
        ...
        try {
            ...
            List<Fine> fineListOld = persistentSocio.getFineList();
            List<Fine> fineListNew = socio.getFineList();
            ...
            for (Fine fineListOldFine : fineListOld) {
                if (!fineListNew.contains(fineListOldFine)) {
                    ...
                    illegalOrphanMessages.add("You must retain Fine " + fineListOldFine + " since its socioId field is not nullable.");
                }
            }
            ...
            for (Fine fineListNewFine : fineListNew) {
                if (!fineListOld.contains(fineListNewFine)) {
                    Socio oldSocioIdOfFineListNewFine = fineListNewFine.getSocioId();
                    fineListNewFine.setSocioId(socio);
                    fineListNewFine = em.merge(fineListNewFine);
                    if (oldSocioIdOfFineListNewFine != null && !oldSocioIdOfFineListNewFine.equals(socio)) {
                        oldSocioIdOfFineListNewFine.getFineList().remove(fineListNewFine);
                        oldSocioIdOfFineListNewFine = em.merge(oldSocioIdOfFineListNewFine);
                    }
                }
            }
    ...
    public void destroy(Integer id) throws IllegalOrphanException, NonexistentEntityException {
        ...
        try {
            ...
            List<Fine> fineListOrphanCheck = socio.getFineList();
            for (Fine fineListOrphanCheckFine : fineListOrphanCheck) {
                if (illegalOrphanMessages == null) {
                    illegalOrphanMessages = new ArrayList<String>();
                }
                illegalOrphanMessages.add("This Socio (" + socio + ") cannot be destroyed since the Fine " + fineListOrphanCheckFine + " in its fineList field has a non-nullable socioId field.");
            }
